...
|---|--:|---|---|
| Time-out voor opnieuw proberen | 90 | Door het systeem afgedwongen Guardrail | Het maximum aantal seconden voordat de meldende motor reageert dat het verzoek te lang duurt om resultaten te retourneren (mogelijk vanwege andere gelijktijdige andere verzoeken); het is mogelijk om opnieuw te verzoeken. |
...
| Rijen per verbinding per jaar | Varieert door toegewezen gegevenscentrum (zie beschrijving voor meer informatie) | Prestatiehandleiding | Rijlimiet per jaar in een verbinding. De onderstaande limieten zijn van kracht vanaf 20 januari 2026 en kunnen in de loop der tijd toenemen om meer rijen per verbinding mogelijk te maken. <p>De rijlimiet varieert, afhankelijk van het toegewezen datacenter, als volgt:</p><ul><li>**VS Azure (gebrek voor de klanten van de V.S.)**: Ongeveer 500 miljard (ongeveer 42 miljard per maand)</li><li>**VS AWS (beschikbaar door verzoek voor klanten in de V.S.)**: Ongeveer 30 miljard (ongeveer 2.5 miljard per maand)</li><li>**Amsterdam**: Ongeveer 200 miljard (ongeveer 16.5 miljard per maand)</li><li>**Alle andere gegevenscentra**: 25 miljard (ongeveer 2 miljard per maand)</li></ul><p>Als u deze limieten wilt overschrijden en problemen met de prestatievermindering wilt voorkomen, neemt u contact op met uw Adobe-accountteam om alternatieve configuraties te bespreken.</p><p>Organisaties kunnen hun datacenter kiezen bij de eerste implementatie van AEP+Apps. Dit besluit is gewoonlijk gebaseerd op de eigen vereisten van de gegevensingezetenschap van elke klant. Bij het kiezen van een datacenter moet rekening worden gehouden met alle gebruiksgevallen van AEP+Apps (niet alleen rijvolumes in Customer Journey Analytics).</p><p>Voor informatie over hoe te om te bekijken welk gegevenscentrum u aan wordt toegewezen, zie [ Customer Journey Analytics het ontvangen plaatsen ](/help/technotes/data-centers.md).</p> |
...
| Naam | Waarde | Limiettype | Beschrijving |
|---|--:|---|---|
| Datasets met titels | 15 - 50 | Door het systeem afgedwongen Guardrail | Maximum aantal gestikte datasets per klant. De waarde is afhankelijk van het Customer Journey Analytics-pakket (zie Productbeschrijving). |
| Lengte achtergrondvulling | 6 - 25 | Door het systeem afgedwongen Guardrail | Maximumaantal maanden back-upgegevens. De waarde is afhankelijk van het Customer Journey Analytics-pakket (zie Productbeschrijving). |
| Frequentie voor terugzoeken/opnieuw afspelen | 01-30-7 | Door het systeem afgedwongen Guardrail | Maximum terugkijkvenster in dagen/Replay frequentie. De waarde is afhankelijk van het Customer Journey Analytics-pakket (zie Productbeschrijving). |
...
| Berekende standaarden | 50.000 | Door het systeem afgedwongen Guardrail | Maximumaantal berekende metriek voor een organisatie. |
...
| Naam | Waarde | Limiettype | Beschrijving |
|---|--:|---|---|
| Te rapporteren rijen voor zakenlieden (BPP) | 1 miljoen | Prestatiehandleiding | Gemiddeld aantal te rapporteren rijen per 1000 te rapporteren profielen van de ondernemer. |
...
| Vertragingen in realtime rapportage | Verwachte vertraging |
|---|---|
| Edge Network SDK/API&#39;s in de Edge Network | &lt; 7 minuten |
| Streaming connectors | &lt; 17 minuten |
| Adobe Analytics-bronaansluiting | &lt; 17 minuten |
| Andere bronconnectors (inclusief batchgegevens) | &lt; 25 uur |
